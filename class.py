...
class Song:
	...
	def ROC(self, distance, prominence):
		actual = set(self.segments[1:len(self.segments)-1])
...
def main():
	...
	total_f1 = 0
	song_titles = listdir("128bpm/songs")
	for st in song_titles:
		if st == ".DS_Store": 
			continue
		datum = song_to_text(st);
		s = Song("128bpm/songs/"+st, "128bpm/data/"+datum, 128, frame_length, hop_length)
		total_f1 += s.ROC(15, 15)
	print("average f1:", total_f1/len(song_titles))
